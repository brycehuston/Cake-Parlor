<?php
	...
	else
	{
		...
		else
		{
			...
			$supp = mysqli_query($conn, "SELECT Name from product WHERE SuppID = '$suppID'");
			$suppName = mysqli_fetch_row($supp);
			//get required the supplier information from the database
			$result = mysqli_query($conn, "SELECT Descr, CategoryID, CostPrice FROM Product WHERE SuppID = '$suppID'");
			...
			if(mysqli_num_rows($result) > 0)
			{
				...
				while ($row = mysqli_fetch_row($result)) 
				{
					...
					$catRes = mysqli_query($conn,"SELECT Name from supplier WHERE ID = '$catId'");
