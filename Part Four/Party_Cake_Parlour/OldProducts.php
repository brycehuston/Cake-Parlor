<?php
	...
	else
	{
		...
		else
		{
			...
			$supp = mysqli_query($conn, "SELECT Name from product WHERE ID = '$suppID'");
			$suppName = mysqli_fetch_row($supp);
			//get required the supplier information from the database
			$result = mysqli_query($conn, "SELECT Descr, CategoryID, CostPrice FROM Product WHERE ID = '$suppID'");
